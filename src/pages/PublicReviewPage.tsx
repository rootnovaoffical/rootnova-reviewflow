...
export default function PublicReviewPage() {
  ...
  const handleRatingSelect = (r: number) => {
    setRating(r); setParticleTrigger(true); setParticleColor(RATING_OPTIONS[r - 1]?.accent || "#6366f1");
    setTimeout(() => setParticleTrigger(false), 800);
    if (r >= 4) { setConfettiTrigger(false); setTimeout(() => setConfettiTrigger(true), 50); setEmojisTrigger(false); setTimeout(() => setEmojisTrigger(true), 50); }
  };
  ...
  const handleCopy = () => { if (aiReview) navigator.clipboard.writeText(aiReview); showToast("Copied to Clipboard!", "success"); safeInsert("analytics_events", { business_id: business?.id, session_id: sessionId, event_type: "copy_event", metadata: {} }); };
  const handleGooglePost = () => { if (aiReview) navigator.clipboard.writeText(aiReview); showToast("Review copied! Redirecting to Google...", "success"); safeInsert("analytics_events", { business_id: business?.id, session_id: sessionId, event_type: "google_click", metadata: {} }); const url = business?.google_review_url || (business?.google_place_id ? `https://search.google.com/local/writereview?placeid=${business.google_place_id}` : null); if (url) window.open(url, "_blank"); setStage("google"); };
    ...
    <>
      ...
      <div className="min-h-screen flex items-center justify-center p-4 py-8">
        <div className="relative w-full max-w-2xl">
          ...
          {business?.logo_url && <div className="flex justify-center mb-6"><div className="logo-pill w-20 h-20 rounded-2xl p-[2px] transform hover:rotate-3 hover:scale-105 transition-all duration-300"><div className="w-full h-full bg-slate-950 rounded-[14px] overflow-hidden flex items-center justify-center"><img src={business.logo_url} alt={business.name} className="w-full h-full object-cover" /></div></div></div>}
          ...
          {stage === "generating" && (
            <div className="glass-card rounded-3xl p-10 text-center screen-enter">
              <div className="ai-generating rounded-2xl p-8 mb-6">
                ...
                <p className="text-lg font-medium text-white">Creating your review...</p><p className="text-sm text-slate-400 mt-1">Crafting a personalized review from your feedback</p>
              </div>
            </div>
          )}
          ...
          {stage === "result" && (
            <div className="glass-card rounded-3xl p-8 sm:p-10 screen-enter">
              <div className="text-center mb-6"><span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-primary-500/20 text-primary-300 border border-primary-500/30 mb-2"><Sparkles className="w-3.5 h-3.5 text-amber-300" /> AI Review Generated</span><h2 className="text-xl font-bold text-white">Ready to Share!</h2></div>
