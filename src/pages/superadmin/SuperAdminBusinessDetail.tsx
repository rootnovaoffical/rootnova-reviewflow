...
export function SuperAdminBusinessDetail() {
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    if (updErr) toast(updErr.message, "error"); else { toast("Logo updated", "success"); loadData(); }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link to="/superadmin/businesses" className="text-ink-400 hover:text-ink-100">←</Link>
      ...
      {tab === "overview" && (
        <div className="space-y-4">
          ...
          <div className="card space-y-3">
            ...
            <div><p className="label">ReviewFlow URL</p><a href={reviewUrl} target="_blank" rel="noreferrer" className="text-sm text-indigo-400 hover:underline">{reviewUrl}</a></div>
            <div><p className="label">Google Review Destination</p>{googleReviewUrl ? <a href={googleReviewUrl} target="_blank" rel="noreferrer" className="text-sm text-indigo-400 hover:underline">{googleReviewUrl}</a> : <p className="text-sm text-ink-400">Not configured</p>}</div>
          </div>
        </div>
      )}
      {tab === "questions" && (
        <div className="space-y-3">
          {questions.length === 0 ? <EmptyState title="No questions" message="Questions are managed by the partner." /> : questions.map((q) => (
            <div key={q.id} className="card">
              <p className="font-medium text-ink-50">{q.question_text}</p>
              <p className="text-xs text-ink-400">{q.flow_type} · {q.is_required ? "Required" : "Optional"}</p>
