...
export default function PublicReviewPage({ businessSlug }: Props) {
  ...
  async function loadQuestions() {
    if (!business) return;
    const { data } = await supabase.from('questions').select('*').eq('is_active', true).eq('business_id', business.id).order('sort_order', { ascending: true });
  ...
  async function submitAnswers() {
    ...
    for (const q of questions.filter((q) => q.is_required)) {
      if (!answers[q.id] || answers[q.id].trim() === '') { showToast('error', 'Please answer all required questions'); return; }
    }
  ...
  if (error) return (<div className="min-h-screen flex items-center justify-center p-6 relative"><SpatialBackground /><div className="relative z-10 text-center max-w-md"><AlertCircle className="w-12 h-12 text-red-400 mx-auto mb-4" /><h1 className="text-xl font-bold text-white mb-2">Oops!</h1><p className="text-zinc-400">{error}</p></div></div>);
  if (!business) return (<div className="min-h-screen flex items-center justify-center relative"><SpatialBackground /><Loader2 className="w-8 h-8 text-blue-400 animate-spin relative z-10" /></div>);
