...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!businessName && businessId) {
      try {
        const r = await fetch(`${supabaseUrl}/rest/v1/businesses?id=eq.${businessId}&select=name`, { headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` } });
        const b = await r.json();
        if (b[0]?.name) bizName = b[0].name;
      } catch {}
    }
