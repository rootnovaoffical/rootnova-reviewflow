...
export const db = {
  ...
  // Module 16 tables
  listInvoices: () => supabase.from("invoices").select("*, organization:organizations(name)").order("created_at", { ascending: false }),
  listInvoicesByOrg: (orgId: string) => supabase.from("invoices").select("*").eq("organization_id", orgId).order("created_at", { ascending: false }),
  getInvoice: (id: string) => supabase.from("invoices").select("*").eq("id", id).maybeSingle(),
  createInvoice: (data: Record<string, unknown>) => supabase.from("invoices").insert(data).select().single(),
  updateInvoice: (id: string, updates: Record<string, unknown>) => supabase.from("invoices").update(updates).eq("id", id),
  deleteInvoice: (id: string) => supabase.from("invoices").delete().eq("id", id),
  listUsageRecords: () => supabase.from("usage_records").select("*, organization:organizations(name)").order("period_start", { ascending: false }),
  listUsageByOrg: (orgId: string) => supabase.from("usage_records").select("*").eq("organization_id", orgId).order("period_start", { ascending: false }),
  createUsageRecord: (data: Record<string, unknown>) => supabase.from("usage_records").insert(data).select().single(),
  updateUsageRecord: (id: string, updates: Record<string, unknown>) => supabase.from("usage_records").update(updates).eq("id", id),
  listHealthScores: () => supabase.from("customer_health_scores").select("*, organization:organizations(name)").order("health_score", { ascending: true }),
  getHealthScore: (orgId: string) => supabase.from("customer_health_scores").select("*").eq("organization_id", orgId).maybeSingle(),
  createHealthScore: (data: Record<string, unknown>) => supabase.from("customer_health_scores").insert(data).select().single(),
  updateHealthScore: (id: string, updates: Record<string, unknown>) => supabase.from("customer_health_scores").update(updates).eq("id", id),
  listCSAlerts: () => supabase.from("customer_success_alerts").select("*, organization:organizations(name)").order("created_at", { ascending: false }),
  listCSAlertsByOrg: (orgId: string) => supabase.from("customer_success_alerts").select("*").eq("organization_id", orgId).order("created_at", { ascending: false }),
  createCSAlert: (data: Record<string, unknown>) => supabase.from("customer_success_alerts").insert(data).select().single(),
  updateCSAlert: (id: string, updates: Record<string, unknown>) => supabase.from("customer_success_alerts").update(updates).eq("id", id),
  listMonitoringEvents: (limit = 100) => supabase.from("monitoring_events").select("*").order("created_at", { ascending: false }).limit(limit),
  createMonitoringEvent: (data: Record<string, unknown>) => supabase.from("monitoring_events").insert(data).select().single(),
  updateMonitoringEvent: (id: string, updates: Record<string, unknown>) => supabase.from("monitoring_events").update(updates).eq("id", id),
  listIncidents: () => supabase.from("incidents").select("*").order("started_at", { ascending: false }),
  getIncident: (id: string) => supabase.from("incidents").select("*").eq("id", id).maybeSingle(),
  createIncident: (data: Record<string, unknown>) => supabase.from("incidents").insert(data).select().single(),
  updateIncident: (id: string, updates: Record<string, unknown>) => supabase.from("incidents").update(updates).eq("id", id),
  listDeploymentChecks: () => supabase.from("deployment_checks").select("*").order("check_category", { ascending: true }),
  createDeploymentCheck: (data: Record<string, unknown>) => supabase.from("deployment_checks").insert(data).select().single(),
  updateDeploymentCheck: (id: string, updates: Record<string, unknown>) => supabase.from("deployment_checks").update(updates).eq("id", id),
  listEntitlements: () => supabase.from("plan_entitlements").select("*, plan:plans(name)").order("sort_order", { ascending: true }),
  listEntitlementsByPlan: (planId: string) => supabase.from("plan_entitlements").select("*").eq("plan_id", planId).order("sort_order", { ascending: true }),
  createEntitlement: (data: Record<string, unknown>) => supabase.from("plan_entitlements").insert(data).select().single(),
  updateEntitlement: (id: string, updates: Record<string, unknown>) => supabase.from("plan_entitlements").update(updates).eq("id", id),
  deleteEntitlement: (id: string) => supabase.from("plan_entitlements").delete().eq("id", id),
  listFlagOverrides: () => supabase.from("feature_flag_overrides").select("*").order("created_at", { ascending: false }),
  createFlagOverride: (data: Record<string, unknown>) => supabase.from("feature_flag_overrides").insert(data).select().single(),
  deleteFlagOverride: (id: string) => supabase.from("feature_flag_overrides").delete().eq("id", id),
};
