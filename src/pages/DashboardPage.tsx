...
import {
  ...
  Settings, LogOut, ChevronDown, Menu, X, Bell
} from 'lucide-react';
...
interface NavItem { id: string; label: string; icon: typeof Star; roles?: string[]; }
interface NavGroup { id: string; label: string; icon: typeof Star; items: NavItem[]; roles?: string[]; }

const NAV_GROUPS: NavGroup[] = [
  {
    id: 'core', label: 'Core', icon: LayoutDashboard, items: [
  ...
  {
    id: 'messaging', label: 'Messaging', icon: Mail, items: [
  ...
  {
    id: 'growth', label: 'Growth', icon: Target, items: [
      { id: 'goals', label: 'Goals', icon: Target },
      { id: 'campaigns', label: 'Campaigns', icon: BarChart3 },
    ]
  },
  {
    id: 'ai', label: 'AI Engine', icon: Brain, items: [
  ...
  {
    id: 'billing', label: 'Billing', icon: CreditCard, roles: ['super_admin', 'partner_admin'], items: [
  ...
  {
    id: 'enterprise', label: 'Enterprise', icon: Building2, roles: ['super_admin', 'partner_admin'], items: [
  ...
  {
    id: 'platform', label: 'Platform', icon: Shield, roles: ['super_admin'], items: [
...
export default function DashboardPage() {
  ...
  function renderModule() {
    if (!business) return <div className="text-center py-12 text-zinc-500">No business selected.</div>;
    ...
    switch (activeView) {
      ...
      case 'integration_providers': return <IntegrationProvidersModule businessId={bid} />;
      ...
      case 'plans': return <PlansModule businessId={bid} />;
  ...
  const activeLabel = (() => {
    for (const g of visibleGroups) { const item = g.items.find((i) => i.id === activeView); if (item) return item.label; }
    return 'Overview';
  })();
    ...
    <div className="min-h-screen relative">
      ...
      <aside className={`fixed top-0 left-0 z-40 h-full w-64 bg-zinc-950/95 backdrop-blur-xl border-r border-white/10 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex items-center gap-2 px-5 h-16 border-b border-white/10">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-400 to-cyan-500 flex items-center justify-center"><Star className="w-5 h-5 text-white" /></div>
        ...
        <nav className="overflow-y-auto h-[calc(100%-4rem)] py-4 px-3 space-y-6">
          {visibleGroups.map((group) => (
            <div key={group.id}>
              <p className="px-3 mb-2 text-[10px] font-semibold text-zinc-600 uppercase tracking-wider flex items-center gap-1.5"><group.icon className="w-3 h-3" /> {group.label}</p>
              <div className="space-y-0.5">
                {group.items.map((item) => (
                  <button key={item.id} onClick={() => { setActiveView(item.id); setSidebarOpen(false); }} className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${activeView === item.id ? 'bg-blue-500/20 text-blue-200 border border-blue-400/20' : 'text-zinc-400 hover:bg-white/5 hover:text-white'}`}>
      ...
      <div className="lg:ml-64">
        ...
        <header className="sticky top-0 z-20 h-16 bg-zinc-950/80 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-white/5 text-zinc-400"><Menu className="w-5 h-5" /></button>
            ...
            {businesses.length > 0 && (
              <div className="relative">
                <button onClick={() => setBizDropdown(!bizDropdown)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-sm text-zinc-200 hover:bg-white/10 transition-colors">
                  ...
                  <span className="text-xs text-zinc-500 capitalize">· {role.replace('_', ' ')}</span>
                ...
                {bizDropdown && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-64 max-h-72 overflow-y-auto rounded-xl bg-zinc-900/95 backdrop-blur-xl border border-white/10 shadow-2xl z-20 py-1.5">
                      {businesses.map((b) => (
                        <button key={b.id} onClick={() => { switchBusiness(b.id); setBizDropdown(false); }} className={`w-full flex items-center gap-2 px-3 py-2 text-sm text-left hover:bg-white/5 transition-colors ${b.id === business?.id ? 'text-blue-300' : 'text-zinc-300'}`}>
        ...
        {/* Content */}
        <main className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">{renderModule()}</main>
      </div>
    </div>
  );
}
...
function NoOrg() {
  return <div className="text-center py-12 text-zinc-500">This module requires an organization. Select a business that belongs to an organization.</div>;
}
