...
export function OrganizationsModule({ organizationId }: Props) {
  return <DataManager table="organizations" organizationId={organizationId} columns={orgColumns} defaultValues={{ status: 'active' }} />;
}

export function OrganizationMembersModule({ organizationId }: Props) {
  return <DataManager table="organization_members" organizationId={organizationId} columns={memberColumns} defaultValues={{ role: 'staff' }} />;
}

export function EnterpriseBranchesModule({ organizationId }: Props) {
  return <DataManager table="enterprise_branches" organizationId={organizationId} columns={branchColumns} defaultValues={{ is_active: true }} />;
}

export function EnterpriseRegionsModule({ organizationId }: Props) {
  return <DataManager table="enterprise_regions" organizationId={organizationId} columns={regionColumns} />;
}

export function OrganizationPoliciesModule({ organizationId }: Props) {
  return <DataManager table="organization_policies" organizationId={organizationId} columns={policyColumns} defaultValues={{ is_enforced: false }} />;
}
