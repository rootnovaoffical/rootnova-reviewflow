...
interface QuestionRow { id: string; question_text: string; question_type: string; options: string[] | null; is_required: boolean; flow_type: string; sort_order: number; }
...
export default function PublicReviewPage({ businessSlug }: Props) {
  ...
  async function loadQuestions() {
    try {
      const { data, error: err } = await supabase.from('questions').select('*').eq('is_active', true).eq('business_id', business!.id).order('sort_order', { ascending: true });
      if (err) throw err;
  ...
  async function startSession() {
    ...
    try {
      ...
      const res = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}` }, body: JSON.stringify({ sessionId: (data as { id: string }).id }) });
      ...
      setStep('result');
    } catch (e) { showToast('error', `Failed to start review session: ${(e as Error).message}`); }
  ...
  function submitAnswers() {
    ...
    for (const q of required) { if (!answers[q.id] || answers[q.id].trim() === '') { showToast('error', 'Please answer all required questions'); return; } }
    startSession();
  }
        ...
        <div className="rounded-2xl bg-zinc-900/80 backdrop-blur-xl border border-white/10 p-6 sm:p-10 shadow-2xl">
          ...
          {step === 'welcome' && (
            <div className="text-center">
              ...
              <button onClick={() => setStep('rating')} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-blue-500/20 border border-blue-400/30 text-blue-200 hover:bg-blue-500/30 font-medium transition-colors">Start Review <ArrowRight className="w-4 h-4" /></button>
            </div>
          )}
          {step === 'rating' && (
            <div className="text-center">
              ...
              <div className="grid grid-cols-5 gap-2 sm:gap-4">
                {[5, 4, 3, 2, 1].map((r) => (
                  <button key={r} onClick={() => handleRating(r)} className="group flex flex-col items-center gap-2 transition-all hover:scale-110">
                    <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center text-2xl sm:text-3xl ${r >= 4 ? 'bg-emerald-500/20 border border-emerald-400/30' : r === 3 ? 'bg-blue-500/20 border border-blue-400/30' : 'bg-red-500/20 border border-red-400/30'}`}>{r === 5 ? '🤩' : r === 4 ? '😀' : r === 3 ? '😐' : r === 2 ? '😕' : '😡'}</div>
              ...
              <div className="space-y-5">
                {questions.length === 0 ? (
                  ...
                  <div key={q.id}>
                    ...
                    {q.question_type === 'text' ? <input type="text" value={answers[q.id] ?? ''} onChange={(e) => handleAnswer(q.id, e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-zinc-500 focus:outline-none focus:border-blue-400/50" placeholder="Your answer…" />
                    : q.question_type === 'multiple_choice' ? (
                      <select value={answers[q.id] ?? ''} onChange={(e) => handleAnswer(q.id, e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:border-blue-400/50"><option value="">— Select —</option>{(q.options || []).map((o) => <option key={o} value={o} className="bg-zinc-900">{o}</option>)}</select>
                    ) : (
                      <div className="flex gap-2">{[5, 4, 3, 2, 1].map((r) => <button key={r} onClick={() => handleAnswer(q.id, String(r))} className={`w-10 h-10 rounded-lg border ${answers[q.id] === String(r) ? 'bg-amber-500/30 border-amber-400/50 text-amber-200' : 'bg-white/5 border-white/10 text-zinc-400 hover:bg-white/10'}`}><Star className="w-5 h-5 mx-auto" /></button>)}</div>
                    )}
                  </div>
                ))}
              </div>
              <button onClick={submitAnswers} disabled={loading} className="mt-6 w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-blue-500/20 border border-blue-400/30 text-blue-200 hover:bg-blue-500/30 font-medium transition-colors disabled:opacity-50">{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Continue <ArrowRight className="w-4 h-4" /></>}</button>
            </div>
          )}
          {step === 'generating' && (
          ...
          {step === 'result' && (
            <div className="text-center">
              <div className="w-16 h-16 rounded-full bg-emerald-500/20 border border-emerald-400/30 flex items-center justify-center mx-auto mb-4"><Star className="w-8 h-8 text-emerald-400" /></div>
              <h2 className="text-xl font-bold text-white mb-4">Your Review is Ready!</h2>
              <div className="rounded-xl bg-white/5 border border-white/10 p-5 text-left mb-6"><p className="text-zinc-200 leading-relaxed whitespace-pre-wrap">{review}</p></div>
              <div className="flex gap-3 justify-center">
                <button onClick={() => { setStep('welcome'); setRating(null); setAnswers({}); setReview(null); setSessionId(null); }} className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-zinc-300 hover:bg-white/10 text-sm">Submit Another</button>
                <a href="#" className="px-5 py-2.5 rounded-xl bg-blue-500/20 border border-blue-400/30 text-blue-200 hover:bg-blue-500/30 text-sm font-medium">Post to Google</a>
              </div>
            </div>
          )}
        </div>
