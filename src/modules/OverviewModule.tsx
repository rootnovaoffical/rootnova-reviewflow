...
interface OverviewModuleProps { businessId: string; }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-white/[0.03] border border-white/10 p-5">
        <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
          <Brain className="w-4 h-4 text-blue-400" /> Recent Reviews
        </h3>
          ...
          <div className="space-y-3">
            {recentReviews.map((r) => (
              <div key={r.id} className="flex items-start gap-3 pb-3 border-b border-white/5 last:border-0">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${r.rating && r.rating >= 4 ? 'bg-emerald-500/10 text-emerald-400' : r.rating && r.rating <= 2 ? 'bg-red-500/10 text-red-400' : 'bg-zinc-500/10 text-zinc-400'}`}>
                  {r.rating ?? '—'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-zinc-300 truncate">{r.ai_generated_review ?? 'No AI review generated'}</p>
                  <p className="text-xs text-zinc-500 mt-0.5">{new Date(r.created_at).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
