...
export default function WorkflowTemplatesModule({ businessId }: Props) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data, error } = await supabase.from('workflow_templates').select('*').eq('is_active', true).order('name');
      ...
      {templates.map((tpl) => (
        <div key={tpl.id} className="rounded-xl bg-white/[0.03] border border-white/10 p-5 hover:border-blue-400/30 transition-colors">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => instantiateTemplate(tpl)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/20 border border-blue-400/30 text-blue-200 hover:bg-blue-500/30 text-xs font-medium transition-colors">
