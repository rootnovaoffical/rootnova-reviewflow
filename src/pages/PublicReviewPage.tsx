      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        {step === 'questions' && (
          <div className="rounded-2xl bg-white/[0.03] border border-white/10 backdrop-blur-xl p-8 shadow-2xl">
            ...
            {questions.length === 0 ? (
              <p className="text-zinc-400 text-sm mb-6">No additional questions. Let's generate your review!</p>
            ) : (
            ...
            <div className="flex justify-between">
              <button onClick={() => setStep('rating')} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-zinc-300 hover:bg-white/10 text-sm">
                <ArrowLeft className="w-4 h-4" /> Back
              </button>
              <button onClick={() => generateReview(false)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500/20 border border-blue-400/30 text-blue-200 hover:bg-blue-500/30 text-sm font-medium">
                Generate Review <Sparkles className="w-4 h-4" />
              </button>
            </div>
          </div>
        )}
        ...
        {step === 'result' && (
          <div className="rounded-2xl bg-white/[0.03] border border-white/10 backdrop-blur-xl p-8 shadow-2xl">
            ...
            <div className="rounded-xl bg-white/5 border border-white/10 p-4 mb-5">
              <p className="text-zinc-200 text-sm leading-relaxed whitespace-pre-wrap">{aiReview}</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button onClick={copyReview} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-zinc-300 hover:bg-white/10 text-sm">
                {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />} {copied ? 'Copied!' : 'Copy'}
              </button>
              <button onClick={() => generateReview(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-zinc-300 hover:bg-white/10 text-sm">
